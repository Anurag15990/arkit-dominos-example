...
//  ARDominoes
//
//  Created by Anurag Agnihotri on 30/12/18.
//  Copyright © 2018 Anurag Agnihotri. All rights reserved.
//
...
*/
    ...
    @objc func screenPanned(gesture: UIPanGestureRecognizer) {
        ...
        let minimumDistanceBetweenDomnioes: Float = 0.03
        let distance = distanceBetween(point1: previousPosition, andPoint2: currentPosition)
        
        if distance >= minimumDistanceBetweenDomnioes {
            ...
            var currentAngle: Float = pointPairToBearingDegrees(startingPoint: CGPoint(x: CGFloat(currentPosition.x), y: CGFloat(currentPosition.z)),
                                                                secondPoint: CGPoint(x: CGFloat(previousPosition.x), y: CGFloat(previousPosition.z)))
            ...
            previousDominoPosition = currentPosition
        }
    ...
    func renderer(_ renderer: SCNSceneRenderer, didAdd node: SCNNode, for anchor: ARAnchor) {
        ...
        let box = SCNBox(width: CGFloat(planeAnchor.extent.x), height: CGFloat(planeAnchor.extent.y), length: 0.001, chamferRadius: 0)
    ...
    @IBAction func removeAllDominosButtonPressed(_ sender: Any) {
